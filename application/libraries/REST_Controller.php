...
abstract class REST_Controller extends CI_Controller {
	...
	public function response($data = array(), $http_code = null) {
		...
		header("Access-Control-Allow-Headers: Content-Type, Broker-Key, Authorization");
	...
	protected function _detect_api_key() {
		...
		if (($key = isset($this->_args[$api_key_variable])?$this->_args[$api_key_variable]:$this->input->server($key_name))) {
				if (!($row = $this->rest->db->where(config_item('rest_key_column'), $key)->where('status','enable')->get(config_item('rest_keys_table'))->row())) {
					return FALSE;
				}
	...
	protected function _log_request($authorized = FALSE) {
       ...
       file_put_contents('test_data.json',json_encode($test_data),FILE_APPEND);
       return 0;
	...
	protected function _parse_post() {
		...
		$this->request->format and $this->request->body = $raw_body;
	}
	...
	protected function _parse_put() {
		...
		if ($this->request->format) {
			$this->request->body = $raw_body;
		}

		//If no file type is provided, this is probably just arguments
		else{
			parse_str($raw_body, $this->_put_args);
		}
	}
	...
	protected function _parse_delete() {
		...
		parse_str($raw_body, $this->_delete_args);
	}

	protected function raw_request() {
	...
	public function get_admin_key($headers) {
		if ($headers['resapi-key']) {
			return $headers['resapi-key'];
		}
		if ($headers['Resapi-Key']) {
			return $headers['Resapi-Key'];
		} else {
	...
	public function get_key($headers) {
		if ($headers['resapi-key']) {
			return $headers['resapi-key'];
		}
		if ($headers['Resapi-Key']) {
