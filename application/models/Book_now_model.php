...
class Book_now_model extends CI_Model
{
	...
	public function getAllpostsForDropdown(){
		$this->db->select("*");
		$query = $this->db->get('retreats');
		if($query->num_rows() > 0){
	...
	{
		$this->db->where('id',$id);
		$this->db->update('book_now',$data);
		return true;
	}
	public function delete($id)
	{
		$this->db->where('id', $id);
		$this->db->delete('book_now');
	}
	...
	public function getAllBook_now(){
		$this->db->select("*");
		$query = $this->db->get('book_now');
		if($query->num_rows() > 0){
			return $query->result();
	...
	public function getAllBook_nowretreats($id){
		$this->db->select("*");
