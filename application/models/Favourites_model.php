...

{
	...
	public function getAllFavouritesOne(){
		$this->db->select("*");
		$query = $this->db->get('favourites');
		if($query->num_rows() > 0){
			return $query->result();
	...
	public function videoallowtowatch($user_id){
	...
	public function getAllFavouritesvidoesofuser($id){
		$this->db->where('id',$id);
		$this->db->select("*");
	...
	public function getAllretreatsforuser($id){
		$this->db->where('id',$id);
		$this->db->select("*");
