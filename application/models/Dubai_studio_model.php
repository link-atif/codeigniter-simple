<?php
class Dubai_studio_model extends CI_Model
{
	...
	{
		$this->db->insert('dubai_studio',$data);
	...
	public function getAll($data,$limit='',$start=''){	
		...
		$this->db->select('*');
		$query = $this->db->get('dubai_studio');
		if($query->num_rows())
		{
			return $query->result();
		}
	...
	{
		$this->db->where('id',$id);
		$this->db->update('dubai_studio',$data);
		return true;
	}
	public function delete($id)
	{
		$this->db->where('id', $id);
		$this->db->delete('dubai_studio');
	}
	...
	public function getAllDubai_studio(){
		$this->db->select("*");
		$query = $this->db->get('dubai_studio');
		if($query->num_rows() > 0){
			return $query->result();
